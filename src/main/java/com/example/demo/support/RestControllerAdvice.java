...
public class RestControllerAdvice implements ResponseBodyAdvice<Object> {
   ...
   *
   * <p>this kind exceptions define for business error, so response 200 with error message
   *
   ...
   *
   * <p>this exception indicate that client pass a wrong parameter, so response 400 for client error
   *
   ...
   *
   * <p>finally in the end handle all the undeclared run time exception, print error log and
   * response 500 for server error then client will know server's status and deal with this
   * situation correctly
   *
